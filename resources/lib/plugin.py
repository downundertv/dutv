...
def _auto_update():
    ...
    RAW = 'https://raw.githubusercontent.com/downundertv/dutv/master/resources/lib/'
    files = ['api.py', 'plugin.py', 'constants.py']
    for f in files:
        try:
            dest = os.path.join(ADDON_PATH, 'resources', 'lib', f)
...
def playlist(output, **kwargs):
    ...
    with codecs.open(output, 'w', encoding='utf8') as f:
        ...
        for elem in sorted(data['liveChannel'], key=lambda e: e['order']):
            ...
            # Use iptv-org xmltv_id when available, fall back to channelCode
            epg_id = EPG_ID_MAP.get(elem['channelCode'], elem['channelCode'])

            f.write(u'\n#EXTINF:-1 tvg-id="{id}" tvg-chno="{channel}" channel-id="{channel}" group-title="{group}" tvg-name="{name}" tvg-logo="{logo}",{name}\n{url}'.format(
                id=epg_id, channel=elem['channelId'], logo=gui.get_art_url(_image('{id}:{site_id}:CHANNEL:IMAGE'.format(id=elem['id'], site_id=LIVE_SITEID), fragment=elem['title'])),
